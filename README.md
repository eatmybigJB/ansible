...
def send(jwt_token, url, phone_number, decrypted_plaintext, ca_cert_path):
    mdpconfig.send_headers['X-HSBC-E2E-Trust-Token'] = jwt_token
    mdpconfig.send_payload['phoneNumber'] = phone_number
    mdpconfig.send_payload['additionalDetails'][0]['value'] = decrypted_plaintext

    response = requests.post(url=url, json=mdpconfig.send_payload, headers=mdpconfig.send_headers, verify=ca_cert_path)

    if response.status_code == 200:
        print(f"{color_text('Success Response: ', 'yellow')}: {color_text(response.json(), 'green')}")
        print(mdpconfig.send_payload)
        return response
    else:
...
def lambda_handler(event, context):
    try:
        ...
        try:
            decrypted_plaintext = get_encrypted_code(encrypted_code, mdpconfig.key_arn)
        ...
        if triggerSource == 'CustomSMSSender_Authentication':
            ...
            try:
                jwt_token = get_one_token(mdpconfig.one_token_url, mdpconfig.one_token_headers, mdpconfig.one_token_payload, ca_cert_path)
                send(jwt_token, mdpconfig.send_url, phone_number, decrypted_plaintext, ca_cert_path)
            except Exception as e:
